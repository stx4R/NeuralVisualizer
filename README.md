...
브라우저에서 다층 퍼셉트론이 학습하는 과정을 실시간으로 들여다보는 도구입니다.
**외부 라이브러리를 하나도 쓰지 않았습니다.** 프레임워크도, 수학 라이브러리도, 3D 엔진도 없습니다.
행렬 연산, 역전파, 3D 원근 투영, 램버트 조명, 은면 처리까지 전부 직접 구현했고, 그리기는 Canvas 2D만 씁니다.
결정 경계·망 구조·손실 곡선·손실 표면 네 가지 시각화가 같은 학습 루프를 공유합니다.
...
그 다음 브라우저에서 `http://localhost:8123/` 를 엽니다.

## 구현한 수식 — 학습

행렬은 `(특징 × 샘플)` 배치입니다. 열 하나가 표본 하나입니다.
...
기울기가 최소 4분의 1로 줄고, 은닉층 두 개를 거치면 16분의 1 이하가 되기 때문입니다.
...
| epoch | 안쪽 r<0.33 | 중간 0.33~0.66 | 바깥 r>0.66 |
|---|---|---|---|
| 500 | 72.6 | 25.0 | 81.2 |
| 1000 | 72.6 | 16.3 | 83.4 |
| 2000 | 61.5 | 20.3 | 84.6 |
...
아르키메데스 나선은 `r ∝ θ` 라 반지름이 클수록 두 갈래 사이 간격이 넓어지므로 바깥쪽이 쉽고,
안쪽으로 갈수록 간격이 좁아 더 많은 굴곡이 필요합니다. 중간 띠가 20%대에 머무는 구간은
단순히 못 맞히는 게 아니라 **반대로 맞히고 있다**는 뜻입니다 — 바깥쪽 경계를 먼저 세우느라
중간 띠를 통째로 잘못된 쪽에 넘겨두었다가, 3000 epoch 무렵 굴곡이 하나 더 생기면서 뒤집힙니다.
